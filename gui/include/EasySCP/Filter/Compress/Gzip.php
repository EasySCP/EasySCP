 ...
 *
 * @category	ispCP
 * @package		EasySCP_Filter
 * @subpackage	Compress
 * @copyright	2006-2011 by ispCP | http://isp-control.net
 * @author		Laurent Declercq <laurent.declercq@ispcp.net>
 * @version		SVN: $Id: Gzip.php 3762 2011-01-14 08:43:43Z benedikt $
 ...
 */
class EasySCP_Filter_Compress_Gzip {
	 ...
	 * The filter mode must be one of the EasySCP_Filter_Compress_Gzip::FILTER_*
	 * constants.
	...
	public function __construct($mode = self::FILTER_FILE, $compressionLevel = 7) {
		...
		if(extension_loaded('zlib')) {
			if($mode === self::FILTER_BUFFER or $mode === self::FILTER_FILE) {
				...
				'EasySCP_Filter_Compress_Gzip error: Unknown filter mode!'
				);
			}
		} else {
			throw new EasySCP_Exception(
				'EasySCP_Filter_Compress_Gzip error: Zlib Compression library is not loaded!'
			);
		}
		...
		if(in_array(
			...
			throw new EasySCP_Exception(
				'EasySCP_Filter_Compress_Gzip error: Wrong value for compression level!'
			);
		}
	}
	...
	protected function _writeFile($gzipData, $filePath) {
		...
		if(is_dir($directory) && is_writable($directory) &&
			...
			throw new EasySCP_Exception(
				"EasySCP_GzipFilter error: `$filePath` is not a valid " .
