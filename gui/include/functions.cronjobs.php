 ...
 * @param type $adminType
 * @param type $userID
 * @return type
 */
function getCronjobs($adminType, $userID){	
	$sql_query = "
		SELECT 
			admin_name, c.*
		FROM
 ...
 * @return boolean
 */
function deleteCronJob($cron_id){
 ...
 * @return boolean
 */
function toggleCronStatus($cron_id){
 ...
 * @param type $minutes
 */
function genMinuteSelect($tpl,$minutes){
	$cfg = EasySCP_Registry::get('Config');
	$tpl->append(
		array(
			'MINUTE_VALUE'		=> '*',
 ...
 * @param type $hours
 */
function genHourSelect($tpl,$hours){
	$cfg = EasySCP_Registry::get('Config');
	$tpl->append(
		array(
			'HOUR_VALUE'	=> '*',
 ...
 * @param type $weekdays
 */
function genDayOfWeekSelect($tpl,$weekdays){
	$cfg = EasySCP_Registry::get('Config');
	$day_names = array(
		tr('Sunday'),
		tr('Monday'),
 ...
 * @param type $months
 */
function genMonthSelect($tpl,$months){
	$cfg = EasySCP_Registry::get('Config');
	$month_names = array(
		tr('January'),
		tr('February'),
 ...
 * @param type $days
 */
function genDayOfMonthSelect($tpl,$days){
	$cfg = EasySCP_Registry::get('Config');
	$tpl->append(
		array(
			'DOM_VALUE'		=> '*',
 ...
 * @param type $user
 */
function genUserSelect($tpl,$user){
	global $cfg;
	...
	if ($_SESSION['user_type']==='admin'){
		genAdminSelect($tpl,$user);
	}
	
	$rs = getAllowedUsers($_SESSION['user_type']);
 ...
 * @param type $user
 */
function genAdminSelect($tpl,$user){
	global $cfg;
	$tpl->append(
			array(
				'USER_ID'		=> 'root',
 ...
 * @return type
 */
function getAllowedUsers($adminType){
	$sql_query = "
		SELECT
			domain_uid
		FROM
 ...
 * @param type $schedule
 */
function genSimpleSelect($tpl,$schedule){
	$cfg = EasySCP_Registry::get('Config');
	$simple_cron = array(
		'@hourly'	=>	tr('Hourly'),
		'@daily'	=>	tr('Daily'),
