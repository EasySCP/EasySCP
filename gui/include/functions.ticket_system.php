...
function createTicket($user_id, $admin_id, $urgency, $subject, $message,
	...
	if ($userLevel < 1 || $userLevel > 2)
		throw new EasySCP_Exception("User level is not valid!");
...
function closeTicket($ticket_id) {
	...
	set_page_message(tr('Ticket was closed!'), 'info');
}
...
function generateTicketList($tpl, $user_id, $start, $count, $userLevel, $status) {
	...
	if ($rs->recordCount() == 0) {
		...
		set_page_message(tr("You don't have support tickets."), 'info');
	} else {
		$prev_si = $start - $count;
		if ($start == 0) {
 ...
 * @version	1.0.0
 *
 * @param EasySCP_pTemplate $tpl	the Template object
 * @param int $ticket_id		the ID of the ticket to display
 * @param int $user_id			the ID of the user
