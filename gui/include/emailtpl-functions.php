...
function get_welcome_email($admin_id, $admin_type='user') {
	...
	if (!$data['subject']) {
		$data['subject'] = tr('Welcome {USERNAME} to EasySCP!', true);
	}
...
function get_lostpassword_activation_email($admin_id) {
	...
	if (!$data['subject']) {
		$data['subject'] = tr('Please activate your new EasySCP password!', true);
	}

	if (!$data['message']) {
		$data['message'] = tr('
Hello {NAME}!
Use this link to activate your new EasySCP password:

{LINK}

Good Luck with the EasySCP System
The EasySCP Team
...
function get_lostpassword_password_email($admin_id) {
	...
	if (!$data['subject']) {
		$data['subject'] = tr('Your new EasySCP login!', true);
	}
...
Thank you for using EasySCP services.
The EasySCP Team
