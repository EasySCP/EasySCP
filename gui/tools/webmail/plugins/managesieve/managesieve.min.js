...
rcube_webmail.prototype.managesieve_updatelist=function(a,b){this.set_busy(!0);switch(a){case "del":var c=b.id;a=this.filters_list;a.remove_row(this.managesieve_rowid(b.id));a.clear_selection();this.show_contentframe(!1);this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!1);$("tr",this.filters_list.list).each(function(){if("none"==this.style.display)$(this).detach();else{var a=this.id.substr(6);$(this).unbind();a>c&&(this.uid=a-1,$(this).attr("id","rcmrow"+this.uid))}});a.init();
break;case "update":var d=$("#rcmrow"+this.managesieve_rowid(b.id));b.name&&$("td",d).text(b.name);b.disabled?d.addClass("disabled"):d.removeClass("disabled");$("#disabled",$("iframe").contents()).prop("checked",b.disabled);break;case "add":a=this.filters_list;d=$('<tr><td class="name"></td></tr>');$("td",d).text(b.name);d.attr("id","rcmrow"+b.id);b.disabled&&d.addClass("disabled");a.insert_row(d.get(0));a.highlight_row(b.id);this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!0);
break;case "list":a=this.filters_list;b.clear&&a.clear();for(d in b.list){var e=b.list[d];var g=document.createElement("TR");var l=document.createElement("TD");$(l).text(e.name);l.className="name";g.id="rcmrow"+e.id;e["class"]&&(g.className=e["class"]);g.appendChild(l);a.insert_row(g)}b.set?a.highlight_row(b.set):this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!1);break;case "setact":c=this.managesieve_setid(b.name);d=$("#rcmrow"+c);b.active?(b.all&&$("tr",this.gui_objects.filtersetslist).addClass("disabled"),
d.removeClass("disabled")):d.addClass("disabled");break;case "setdel":c=this.managesieve_setid(b.name);this.filtersets_list.remove_row(c);this.filters_list.clear();this.show_contentframe(!1);this.enable_command("plugin.managesieve-setdel","plugin.managesieve-setact","plugin.managesieve-setget",!1);delete this.env.filtersets[c];break;case "setadd":c="S"+(new Date).getTime(),a=this.filtersets_list,d=$('<tr class="disabled"><td class="name"></td></tr>'),$("td",d).text(b.name),d.attr("id","rcmrow"+c),
this.env.filtersets[c]=b.name,a.insert_row(d.get(0)),b.index!=a.rowcount-1&&(d.detach(),b=$("tr:visible",a.list).get(b.index),d.insertBefore(b)),a.select(c),this.managesieve_fixdragend(d)}this.set_busy(!1)};
...
function rule_header_select(a){var b=document.getElementById("header"+a),c=document.getElementById("rule_size"+a),d=document.getElementById("rule_op"+a),e=document.getElementById("custom_header"+a+"_list"),g=document.getElementById("rule_mod"+a),l=document.getElementById("rule_trans"+a),k=document.getElementById("rule_comp"+a),m=document.getElementById("rule_date_part"+a),n=document.getElementById("rule_date_header_div"+a),f=b.value;"size"==f?(c.style.display="inline",$.each([d,e,g,l,k],function(){this.style.display=
"none"})):(e.style.display="..."!=f?"none":"inline-block",c.style.display="none",d.style.display="inline",k.style.display="",g.style.display="body"==f||"currentdate"==f||"date"==f?"none":"block",l.style.display="body"==f?"block":"none");m&&(m.style.display="currentdate"==f||"date"==f?"inline":"none");n&&(n.style.display="date"==f?"":"none");rule_op_select(d,a,f);rule_mod_select(a,f);b.style.width="..."==f?"40px":""}
...
function rule_adv_switch(a,b){b=$(b);var c=b.hasClass("hide");a=$("#rule_advanced"+a);c?(a.hide(),b.removeClass("hide").addClass("show")):(a.show(),b.removeClass("show").addClass("hide"))}
...
rcube_webmail.prototype.managesieve_tip_register=function(a){var b,c=parent.rcmail,d=c?parent.rcmail.env.ms_tip_layer:rcmail.env.ms_tip_layer;for(b in a)$("#"+a[b][0]).data("tip",a[b][1]).bind("mouseenter",function(a){a=$(this);var b=a.offset(),e=b.left,k=b.top-12,m=a.width();c&&(b=$("mail"==rcmail.env.task?"#sievefilterform > iframe":"#filter-box",parent.document).offset(),k+=b.top,e+=b.left);d.html(a.data("tip"));k-=d.height();d.css({left:e,top:k,minWidth:m-2+"px"}).show()}).bind("mouseleave",function(a){d.hide()})};
...
changeMonth:!0,showOtherMonths:!0,selectOtherMonths:!0,onSelect:function(a){$(this).focus().val(a)}}),$("input.datepicker").datepicker());$("#vacation_timefrom, #vacation_timeto").attr("autocomplete","off").autocomplete({delay:100,minLength:1,source:function(a,b){var c=[];for(a=0;24>a;a++)c.push(sieve_formattime(a,0));c.push(sieve_formattime(23,59));return b(c)},open:function(a,b){a=$(this);var c=a.val();b=a.autocomplete("widget").css("width","10em");var d=a.data("ui-autocomplete").menu;c&&c.length&&
b.children().each(function(){var a=$(this);0==a.text().indexOf(c)&&d._scrollIntoView(a)})},select:function(a,b){$(this).val(b.item.value);return!1}}).click(function(){$(this).autocomplete("search",$(this).val()||" ")})}
