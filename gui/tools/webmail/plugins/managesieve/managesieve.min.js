...
rcube_webmail.prototype.managesieve_updatelist=function(a,b){this.set_busy(!0);switch(a){case "del":var c=b.id,d=this.filters_list;d.remove_row(this.managesieve_rowid(b.id));d.clear_selection();this.show_contentframe(!1);this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!1);$("tr",this.filters_list.list).each(function(){if("none"==this.style.display)$(this).detach();else{var a=this.id.substr(6);$(this).unbind();a>c&&(this.uid=a-1,$(this).attr("id","rcmrow"+this.uid))}});d.init();
...
rcube_webmail.prototype.managesieve_tip_register=function(a){var b,c=parent.rcmail,d=c?parent.rcmail.env.ms_tip_layer:rcmail.env.ms_tip_layer;for(b in a)$("#"+a[b][0]).data("tip",a[b][1]).bind("mouseenter",function(a){a=$(this);var b=a.offset(),e=b.left,g=b.top-12,m=a.width();c&&(b=$("mail"==rcmail.env.task?"#sievefilterform > iframe":"#filter-box",parent.document).offset(),g+=b.top,e+=b.left);d.html(a.data("tip"));g-=d.height();d.css({left:e,top:g,minWidth:m-2+"px"}).show()}).bind("mouseleave",function(a){d.hide()})};
