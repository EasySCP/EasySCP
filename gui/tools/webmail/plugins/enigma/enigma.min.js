...
rcube_webmail.prototype.enigma_key_create_save=function(){var a,b,c=[],d=$("#key-pass").val();a=$("#key-pass-confirm").val();var e=$("#key-size").val();$('[name="identity[]"]:checked').each(function(){c.push({name:$(this).data("name")||"",email:$(this).data("email")})});if(!d||!a)return alert(this.get_label("enigma.formerror"));if(d!=a)return alert(this.get_label("enigma.passwordsdiffer"));if(!c.length)return alert(this.get_label("enigma.noidentselected"));window.openpgp&&(window.msCrypto||window.crypto&&
(window.crypto.getRandomValues||window.crypto.subtle))?(b=this.set_busy(!0,"enigma.keygenerating"),a={numBits:e,userIds:c,passphrase:d},openpgp.generateKey(a).then(function(a){rcmail.http_post("plugin.enigmakeys",{_a:"import",_keys:a.privateKeyArmored,_generated:1,_passwd:d,_keyid:a.key.primaryKey.fingerprint},b)},function(a){console.error(a);rcmail.set_busy(!1,null,b);rcmail.display_message(rcmail.get_label("enigma.keygenerateerror"),"error")})):rcmail.display_message(rcmail.get_label("enigma.keygennosupport"),
...
rcube_webmail.prototype.enigma_find_publickey=function(a){this.mailvelope_search_pubkeys([a],function(a){},function(a){var c=rcmail.set_busy(!0,"enigma.importwait");a={_a:"import",_keys:a};"plugin.enigmakeys"==rcmail.env.action&&(a._refresh=1);rcmail.http_post("plugin.enigmakeys",a,c)})};
