...
rcube_webmail.prototype.enigma_key_create_save=function(){var a=$("#key-ident > option").filter(":selected").text();var b=$("#key-pass").val(),c=$("#key-pass-confirm").val(),d=$("#key-size").val();if(!b||!c)return alert(this.get_label("enigma.formerror"));if(b!=c)return alert(this.get_label("enigma.passwordsdiffer"));if(a.match(/^<[^>]+>$/))return alert(this.get_label("enigma.nonameident"));if(window.openpgp&&window.crypto&&(window.crypto.getRandomValues||window.crypto.subtle)){var e=this.set_busy(!0,
"enigma.keygenerating");a={numBits:d,userId:a,passphrase:b};openpgp.generateKeyPair(a).then(function(a){rcmail.http_post("plugin.enigmakeys",{_a:"import",_keys:a.privateKeyArmored},e)},function(a){rcmail.set_busy(!1,null,e);rcmail.display_message(rcmail.get_label("enigma.keygenerateerror"),"error")})}else rcmail.env.enigma_keygen_server?(e=this.set_busy(!0,"enigma.keygenerating"),a={_a:"generate",_user:a,_password:b,_size:d},rcmail.http_post("plugin.enigmakeys",a,e)):rcmail.display_message(rcmail.get_label("enigma.keygennosupport"),
