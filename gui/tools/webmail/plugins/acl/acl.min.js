...
rcube_webmail.prototype.acl_list_update=function(a){$(this.gui_objects.acltable).html(a);this.acl_list_init()};rcube_webmail.prototype.acl_get_usernames=function(){var a=[],c,b,d=this.acl_list,e=d.get_selection();var h=0;for(c=e.length;h<c;h++)if(this.env.acl_specials.length&&0<=$.inArray(e[h],this.env.acl_specials))a.push(e[h]);else if(b=d.rows[e[h]])b=$("td.user",b.obj),1==b.length&&a.push(b.text());return a};
...
rcube_webmail.prototype.acl_add_row=function(a,c){var b=[];var d=[];var e=a.id,h=this.acl_list,f=this.env.acl_advanced?[]:this.env.acl_items,k=$("thead > tr",this.gui_objects.acltable).clone();$("th",k).map(function(){var b=$("<td>"),c=$(this).attr("title"),d=this.className.replace(/^acl/,"");c&&b.attr("title",c);f&&f[d]&&(d=f[d]);"user"==d?b.addClass(d).append($("<a>").text(a.username)):b.addClass(this.className+" "+rcmail.acl_class(a.acl,d)).text("");$(this).replaceWith(b)});k.attr("id","rcmrow"+
e);k=k.get(0);this.env.acl[e]=a.acl;for(g in this.env.acl)this.env.acl[g]&&(this.env.acl_specials.length&&0<=$.inArray(g,this.env.acl_specials)?d.push(g):b.push(g));b.sort();b=d.concat(b);var g=0;for(d=b.length;g<d&&b[g]!=e;g++);g&&g<d?($("#rcmrow"+b[g-1]).after(k),h.init_row(k),h.rowcount++):h.insert_row(k);c&&h.select_row(a.id)};
rcube_webmail.prototype.acl_init_form=function(a){var c,b,d="",e="user",h=$("body");var f=$("#advancedrights");var k=$("#simplerights"),g=$("#acluser"),n=$("#usertype");if(!this.acl_form){var m=function(){$('input[value="user"]').prop("checked",!0)};g.click(m).keypress(m)}this.acl_form=$("#aclform");this.env.acl_advanced?(f.show(),k.hide()):(k.show(),f.hide(),f=k);f=$(":checkbox",f);f.attr("checked",!1);a&&(c=this.acl_list.rows[a])?(c=c.obj,f.map(function(){b=$("td."+this.id,c);b.length&&b.hasClass("enabled")&&
(this.checked=!0)}),!this.env.acl_specials.length||0>$.inArray(a,this.env.acl_specials)?d=$("td.user",c).text():e=a):f.filter(function(){return this.id.match(/^acl([lrs]|read)$/)}).prop("checked",!0);g.val(d);$("input[value="+e+"]").prop("checked",!0);this.acl_id=a;d={};var l=this;h=document.body;d[this.gettext("save")]=function(a){l.command("acl-save")};d[this.gettext("cancel")]=function(a){l.command("acl-cancel")};f=$('<div style="width:480px; min-height:280px"></div>');this.acl_form.appendTo(f).show();
this.acl_popup=this.show_popup_dialog(f,a?this.gettext("acl.editperms"):this.gettext("acl.newuser"),d,{button_classes:["mainaction"],modal:!0,closeOnEscape:!0,close:function(a,b){(l.is_framed()?parent.rcmail:l).ksearch_hide();l.acl_form.appendTo(h).hide();$(this).remove();window.focus()}});"user"==e?g.focus():$("input:checked",n).focus()};
rcube_webmail.prototype.acl_class=function(a,c){var b,d=0;a=String(a);c=String(c);var e=0;for(b=c.length;e<b;e++)-1<a.indexOf(c[e])&&d++;return d==b?"enabled":d?"partial":"disabled"};
