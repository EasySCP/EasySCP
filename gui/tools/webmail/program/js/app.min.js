...
!0),bw.mz&&"application/pdf"==this.env.mimetype?(a=0,$(this.gui_objects.messagepartframe).on("load",function(){if(a++)try{this.contentWindow.document&&f.enable_command("print",!0)}catch(b){}})):this.enable_command("print",!0),this.env.is_message&&(this.enable_command("reply","reply-all","edit","viewsource","forward","forward-inline","forward-attachment",!0),this.env.list_post&&this.enable_command("reply-list",!0)),this.env.mimetype.startsWith("image/"))$(this.gui_objects.messagepartframe).on("load",
...
this.env.extwin&&(a._extwin=1);a=this.url(d?"preview":"show",a);d&&0<=String(g.location.href).indexOf(a)?this.show_contentframe(!0):d||!this.env.message_extwin||this.env.extwin?this.location_href(a,g,!0):this.open_window(a,!0)}};this.set_unread_message=function(a,b){var d=this;d.message_list||(d=d.opener());!d&&window.parent&&(d=parent.rcmail);d&&d.message_list&&(!1===d.set_message(a,"unread",!1)&&d.set_message(a+"-"+b,"unread",!1),0<d.env.unread_counts[b]&&(d.env.unread_counts[b]-=1,d.set_unread_count(b,
...
function(a){f.command("copy",a)});if(a&&a!=this.env.mailbox){var d=this.selection_post_data({_target_mbox:a});d._uid&&this.http_post("copy",d,this.display_message(this.get_label("copyingmessage"),"loading"))}};this.move_messages=function(a,b){if(a&&"object"===typeof a)a=a.id;else if(!a)return this.folder_selector(b,function(a){f.command("move",a)});if(a&&(a!=this.env.mailbox||this.is_multifolder_listing())){var d=!1,e=this.selection_post_data({_target_mbox:a});e._uid&&("show"==this.env.action?d=this.set_busy(!0,
"movingmessage"):this.show_contentframe(!1),this.enable_command(this.env.message_commands,!1),this.with_selected_messages("move",e,d))}};this.delete_messages=function(a){var b=this.message_list,d=this.env.trash_mailbox;if(this.env.flag_for_deletion)return this.mark_message("delete"),!1;d&&this.env.mailbox!=d?this.env.delete_junk&&this.env.junk_mailbox&&this.env.mailbox==this.env.junk_mailbox?this.permanently_remove_messages():b&&b.modkey==SHIFT_KEY||a&&rcube_event.get_modifier(a)==SHIFT_KEY?confirm(this.get_label("deletemessagesconfirm"))&&
this.permanently_remove_messages():this.move_messages(d):this.permanently_remove_messages();return!0};this.permanently_remove_messages=function(){var a=this.selection_post_data();a._uid&&(this.show_contentframe(!1),this.with_selected_messages("delete",a))};this.with_selected_messages=function(a,b,d,e){var g=0,h="delete"==a||!this.is_multifolder_listing();if(this.message_list){var f,l,m,n=[],p=this.message_list.get_selection();f=0;for(len=p.length;f<len;f++)l=p[f],this.env.threading&&(g+=this.update_thread(l),
m=this.message_list.find_root(l),m!=l&&0>$.inArray(m,n)&&n.push(m)),h&&this.message_list.remove_row(l,this.env.display_next&&f==p.length-1);!this.env.display_next&&h&&this.message_list.clear_selection();f=0;for(len=n.length;f<len;f++)this.add_tree_icons(n[f])}0>g?b._count=-1*g:0<g&&h&&this.delete_excessive_thread_rows();h||(b._refresh=1);d||(d=this.display_message(this.get_label("move"==a?"movingmessage":"deletingmessage"),"loading"));this.http_post(e||a,b,d)};this.selection_post_data=function(a){"object"!=
typeof a&&(a={});a._mbox=this.env.mailbox;if(!a._uid){var b=this.env.uid?[this.env.uid]:this.message_list.get_selection();a._uid=this.uids_to_list(b)}this.env.action&&(a._from=this.env.action);this.env.search_request&&(a._search=this.env.search_request);this.env.display_next&&this.env.next_uid&&(a._next_uid=this.env.next_uid);return a};this.mark_message=function(a,b){var d=[],e=[],g,h,f,l=this.message_list;b?d[0]=b:this.env.uid?d[0]=this.env.uid:l&&(d=l.get_selection());if(l)for(l.focus(),h=0,g=d.length;h<
...
this.http_post("mark",g,h)};this.toggle_delete_status=function(a){var b=a.length,d,e,g=!0,h=this.message_list?this.message_list.rows:{};if(1==b)return!this.message_list||h[a[0]]&&!h[a[0]].deleted?this.flag_as_deleted(a):this.flag_as_undeleted(a),!0;for(d=0;d<b;d++)if(e=a[d],h[e]&&!h[e].deleted){g=!1;break}g?this.flag_as_undeleted(a):this.flag_as_deleted(a);return!0};this.flag_as_undeleted=function(a){var b,d=a.length,e=this.selection_post_data({_uid:this.uids_to_list(a),_flag:"undelete"}),g=this.display_message(this.get_label("markingmessage"),
"loading");for(b=0;b<d;b++)this.set_message(a[b],"deleted",!1);this.http_post("mark",e,g)};this.flag_as_deleted=function(a){for(var b=[],d=this.selection_post_data({_uid:this.uids_to_list(a),_flag:"delete"}),e=this.display_message(this.get_label("markingmessage"),"loading"),g=this.message_list,h=g?g.rows:{},f=0,l=0,m=a.length;l<m;l++)uid=a[l],h[uid]&&(h[uid].unread&&(b[b.length]=uid),this.env.skip_deleted?(f+=this.update_thread(uid),g.remove_row(uid,this.env.display_next&&l==g.selection.length-1)):
this.set_message(uid,"deleted",!0));this.env.skip_deleted&&g&&(this.env.display_next&&g.rowcount||g.clear_selection(),0>f?d._count=-1*f:0<f&&this.delete_excessive_thread_rows());b.length&&(d._ruid=this.uids_to_list(b));this.env.skip_deleted&&this.env.display_next&&this.env.next_uid&&(d._next_uid=this.env.next_uid);this.http_post("mark",d,e)};this.flag_deleted_as_read=function(a){var b,d,e,g=this.message_list?this.message_list.rows:{};"string"==typeof a&&(a=a.split(","));d=0;for(e=a.length;d<e;d++)b=
...
d),g=$("li",e);e.height();var h=parseInt(this.value);if(27!=b.which&&9!=b.which&&13!=b.which&&!d.is(":visible"))return f.show_menu("pagejump-selector",!0,b);if("keydown"==b.type)if(40==b.which)g.length>h&&(this.value=h+=1);else if(38==b.which)1<h&&g.length>h-1&&(this.value=h-=1);else{if(13==b.which)return $(this).change();if(27==b.which||9==b.which)return $(a).val(f.env.current_page)}$("li.selected",e).removeClass("selected");(b=$(g[h-1])).length&&(b.addClass("selected"),$("#pagejump-selector").scrollTop(e.height()/
g.length*(h-1)-d.height()/2))}).on("change",function(a){(a=parseInt(this.value))&&a!=f.env.current_page&&!f.busy&&(f.hide_menu("pagejump-selector"),f.list_page(a))})};this.update_pagejumper=function(){$("input.rcpagejumper").val(this.env.current_page).prop("disabled",2>this.env.pagecount)};this.check_mailvelope=function(a){if("undefined"!==typeof window.mailvelope)this.mailvelope_load(a);else $(window).on("mailvelope",function(){f.mailvelope_load(a)})};this.mailvelope_load=function(a){this.env.browser_capabilities&&
...
"toggle-editor"==a.action&&f.enable_command("compose-encrypted",!a.props.html)})}};this.compose_encrypted=function(a){var b,d=$("#"+this.env.composebody).parent();f.mailvelope_editor?(f.mailvelope_editor=null,f.compose_skip_unsavedcheck=!1,f.set_button("compose-encrypted","act"),d.removeClass("mailvelope").find("iframe:not([aria-hidden=true])").remove(),$("#"+f.env.composebody).show(),$("[name='_pgpmime']").remove(),f.enable_command("spellcheck","insert-sig","toggle-editor","insert-response","save-response",
!0),f.triggerEvent("compose-encrypted",{active:!1})):(this.spellcheck_state()&&this.editor.spellcheck_stop(),b=a.quotedMail?{quotedMail:a.quotedMail,quotedMailIndent:!1}:{predefinedText:$("#"+this.env.composebody).val()},"reply"==this.env.compose_mode&&(b.quotedMailIndent=!0,b.quotedMailHeader=this.env.compose_reply_header),mailvelope.createEditorContainer("#"+d.attr("id"),f.mailvelope_keyring,b).then(function(a){f.mailvelope_editor=a;f.compose_skip_unsavedcheck=!0;f.set_button("compose-encrypted",
"sel");d.addClass("mailvelope");$("#"+f.env.composebody).hide();f.enable_command("spellcheck","insert-sig","toggle-editor","insert-response","save-response",!1);f.triggerEvent("compose-encrypted",{active:!0});f.env.attachments&&!$.isEmptyObject(f.env.attachments)&&(alert(f.get_label("encryptnoattachments")),$.each(f.env.attachments,function(a,b){f.remove_from_attachment_list(a)}))},function(a){console.error(a);console.log(b)}))};this.mailvelope_submit_messageform=function(a,b){var d=[];$.each(["to",
"cc","bcc"],function(a,b){for(var e,f=$.trim($('[name="_'+b+'"]').val());f.length&&rcube_check_email(f,!0);)e=RegExp.$2,d.push(e),f=f.substr(f.indexOf(e)+e.length+1).replace(/^\s*,\s*/,"")});var e=0<d.length;f.mailvelope_keyring.validKeyForAddress(d).then(function(g){var h=[];$.each(g,function(a,b){!1===b&&(e=!1,h.push(a))});if(!e&&h.length)return f.simple_dialog(f.get_label("nopubkeyfor").replace("$email",h.join(", "))+"<p>"+f.get_label("searchpubkeyservers")+"</p>","encryptedsendialog",function(){f.mailvelope_search_pubkeys(h,
function(){return!0})},{button:"search"}),!1;if(!e)return d.length||(alert(f.get_label("norecipientwarning")),$("[name='_to']").focus()),!1;var k=[],l=f.env.identities[$("[name='_from'] option:selected").val()];$.each(f.env.identities,function(a,b){k.push(b.email)});f.mailvelope_keyring.validKeyForAddress(k).then(function(e){valid_sender=null;$.each(e,function(a,b){if(!1!==b&&(valid_sender=a,valid_sender==l))return!1});if(!valid_sender&&!confirm(f.get_label("nopubkeyforsender")))return!1;d.push(valid_sender);
f.mailvelope_editor.encrypt(d).then(function(d){var e=f.gui_objects.messageform,g=$("[name='_pgpmime']",e),h=f.set_busy(!0,a||b?"savingmessage":"sendingmessage");e.target="savetarget";e._draft.value=a?"1":"";e.action=f.add_url(e.action,"_unlock",h);e.action=f.add_url(e.action,"_framed",1);b&&(e.action=f.add_url(e.action,"_saveonly",1));g.length||(g=$('<input type="hidden" name="_pgpmime">').appendTo(e));g.val(d);e.submit()},function(a){console.log(a)})},function(a){console.error(a)})},function(a){console.error(a)});
...
function(a,b,d){var e=[],g=new PublicKey,h=f.display_message(f.get_label("loading"),"loading");$.each(a,function(a,b){var d=$.Deferred();g.search(b,function(a,e){null!==e?d.resolve([b]):d.resolve([b].concat(a))});e.push(d)});$.when.apply($,e).then(function(){var a=[],e=[];$.each(arguments,function(b,d){var g=d.shift();d.length?e=e.concat(d):a.push(g)});f.hide_message(h);b(!0);e.length&&f.mailvelope_key_import_dialog(e,d);a.length&&f.display_message(f.get_label("nopubkeyfor").replace("$email",a.join(", ")),
"warning")}).fail(function(){console.error("Pubkey lookup failed with",arguments);f.hide_message(h);f.display_message("pubkeysearcherror","error");b(!1)})};this.mailvelope_key_import_dialog=function(a,b){var d=$("<div>").addClass("listing pgpkeyimport");$.each(a,function(a,b){var h=$("<div>").addClass("key");b.revoked&&h.addClass("revoked");b.disabled&&h.addClass("disabled");b.expired&&h.addClass("expired");h.append($("<label>").addClass("keyid").text(f.get_label("keyid")));h.append($("<a>").text(b.keyid.substr(-8).toUpperCase()).attr("href",
b.info).attr("target","_blank").attr("tabindex","-1"));h.append($("<label>").addClass("keylen").text(f.get_label("keylength")));h.append($("<span>").text(b.keylen));b.expirationdate&&(h.append($("<label>").addClass("keyexpired").text(f.get_label("keyexpired"))),h.append($("<span>").text((new Date(1E3*b.expirationdate)).toDateString())));b.revoked&&h.append($("<span>").addClass("keyrevoked").text(f.get_label("keyrevoked")));var k=$("<ul>").addClass("uids");$.each(b.uids,function(a,b){var d=$("<li>").addClass("uid");
b.revoked&&d.addClass("revoked");b.disabled&&d.addClass("disabled");b.expired&&d.addClass("expired");k.append(d.text(b.uid))});h.append(k);h.append($("<input>").attr("type","button").attr("rel",b.keyid).attr("value",f.get_label("import")).addClass("button importkey").prop("disabled",b.revoked||b.disabled||b.expired));d.append(h)});f.show_popup_dialog($("<div>").append($("<p>").html(f.get_label("encryptpubkeysfound"))).append(d),f.get_label("importpubkeys"),[{text:f.get_label("close"),click:function(){(f.is_framed()?
parent.$:$)(this).dialog("close")}}]);d.on("click","input.button.importkey",function(){var a=$(this),d=a.attr("rel"),h=new PublicKey,k=f.display_message(f.get_label("loading"),"loading");h.get(d,function(h,m){f.hide_message(k);m?f.display_message(f.get_label("keyservererror"),"error"):b?b(h):f.mailvelope_keyring.importPublicKey(h).then(function(b){"REJECTED"!==b&&(b=d.substr(-8).toUpperCase(),a.closest(".key").fadeOut(),f.display_message(f.get_label("keyimportsuccess").replace("$key",b),"confirmation"))},
function(a){console.log(a)})})})};this.expunge_mailbox=function(a){var b,d={_mbox:a};a==this.env.mailbox&&(b=this.set_busy(!0,"loading"),d._reload=1,this.env.search_request&&(d._search=this.env.search_request));this.http_post("expunge",d,b)};this.purge_mailbox=function(a){var b,d={_mbox:a};if(!confirm(this.get_label("purgefolderconfirm")))return!1;a==this.env.mailbox&&(b=this.set_busy(!0,"loading"),d._reload=1);this.http_post("purge",d,b)};this.purge_mailbox_test=function(){return this.env.exists&&
(this.env.mailbox==this.env.trash_mailbox||this.env.mailbox==this.env.junk_mailbox||this.env.mailbox.startsWith(this.env.trash_mailbox+this.env.delimiter)||this.env.mailbox.startsWith(this.env.junk_mailbox+this.env.delimiter))};this.mark_all_read=function(a,b){var d,e,g=[];e=this.message_list;var h=a||this.env.mailbox,k={_uid:"*",_flag:"read",_mbox:h,_folders:b};if("string"!=typeof b){d=this.mark_all_read_state(h);if(!d)return;if(1<d){$.each({cur:1,sub:2,all:4},function(a,b){var e=$("<label>").attr("style",
...
24))};this.init_messageform=function(){if(!this.gui_objects.messageform)return!1;var a,b,d=$("[name='_from']"),e=$("[name='_to']"),g=$("input[name='_subject']"),f=$("[name='_message']").get(0),k="1"==$("input[name='_is_html']").val(),l=["cc","bcc","replyto","followupto"],m,n=this.opener();n&&"compose"==n.env.action&&(setTimeout(function(){1<opener.history.length?opener.history.back():n.redirect(n.get_task_url("mail"))},100),this.env.opened_extwin=!0);0<this.env.autocomplete_threads&&(m={threads:this.env.autocomplete_threads,
sources:this.env.autocomplete_sources});this.init_address_input_events(e,m);for(a in l)this.init_address_input_events($("[name='_"+l[a]+"']"),m);k||(a=this.env.top_posting&&this.env.compose_mode?0:f.value.length,"select-one"==d.prop("type")&&(this.set_caret_pos(f,0),this.change_identity(d[0])),this.set_caret_pos(f,a),a&&$(f).scrollTop(f.scrollHeight));this.env.save_localstorage&&this.compose_restore_dialog(0,k);""==e.val()?b=e:""==g.val()?b=g:f&&(b=f);this.env.compose_focus_elem=$(b).filter(":visible").focus().get(0);
this.compose_field_hash(!0);this.auto_save_start()};this.compose_restore_dialog=function(a,b){var d,e,g,h=this.local_storage_get_item("compose.index",[]),k=function(a){++a<h.length&&f.compose_restore_dialog(a,b)};for(d=a||0;d<h.length;d++)if(e=h[d],g=this.local_storage_get_item("compose."+e,null,!0)){if(g.changed&&e==this.env.compose_id){this.restore_compose_form(e,b);break}if(!(this.env.draft_id&&g.draft_id&&g.draft_id!=this.env.draft_id||this.env.reply_msgid&&g.reply_msgid!=this.env.reply_msgid)&&
g.changed&&g.session!=this.env.session_id){this.show_popup_dialog(this.get_label("restoresavedcomposedata").replace("$date",(new Date(g.changed)).toLocaleString()).replace("$subject",g._subject).replace(/\n/g,"<br/>"),this.get_label("restoremessage"),[{text:this.get_label("restore"),"class":"mainaction",click:function(){f.restore_compose_form(e,b);f.remove_compose_data(e);f.save_compose_form_local();$(this).dialog("close")}},{text:this.get_label("delete"),"class":"delete",click:function(){f.remove_compose_data(e);
...
{name:this.env.contactdata[g],input:d.get(0)};this.http_request("group-expand",{_source:this.env.source,_gid:k},!1)}b.length&&d.length&&(g=d.val(),f=RegExp(RegExp.escape(e)+"\\s*$"),g&&!f.test(g)&&(g+=e+" "),d.val(g+b.join(e+" ")+e+" ").change(),this.triggerEvent("add-recipient",{field:a,recipients:b}));return b.length};this.check_compose_input=function(a){var b,d,e,g=this.env.max_disclosed_recipients,h=$("[name='_to']"),k=$("[name='_cc']"),l=$("[name='_bcc']"),m=$("[name='_from']"),n=$("[name='_subject']"),
p=function(a){a=$.map(a,function(a){a=$.trim(a.val());return a.length?a:null});return a.join(",").replace(/^[\s,;]+/,"").replace(/[\s,;]+$/,"")};if("text"==m.prop("type")&&!rcube_check_email(m.val(),!0))return alert(this.get_label("nosenderwarning")),m.focus(),!1;if(!rcube_check_email(p([h,k,l]),!0))return alert(this.get_label("norecipientwarning")),h.focus(),!1;for(b in this.env.attachments)if("object"===typeof this.env.attachments[b]&&!this.env.attachments[b].complete)return alert(this.get_label("notuploadedwarning")),
!1;if(g&&!this.env.disclosed_recipients_warned&&rcube_check_email(d=p([h,k]),!0,!0)>g){var s=function(b){b&&(b=l.val(),l.val((b?b+", ":"")+d).change(),h.val("").change(),k.val("").change());e.dialog("close");f.check_compose_input(a)&&f.command(a,{nocheck:!0})};e=this.show_popup_dialog(this.get_label("disclosedrecipwarning"),this.get_label("disclosedreciptitle"),[{text:this.get_label("sendmessage"),click:function(){s(!1)},"class":"mainaction"},{text:this.get_label("bccinstead"),click:function(){s(!0)}},
{text:this.get_label("cancel"),click:function(){e.dialog("close")}}],{dialogClass:"warning"});this.env.disclosed_recipients_warned=!0;return!1}if(!this.env.nosubject_warned&&""==n.val()){var t=$("<input>").attr({type:"text",size:40});b=$('<div class="prompt">').append($('<div class="message">').text(this.get_label("nosubjectwarning"))).append(t);s=function(){n.val(t.val());e.dialog("close");f.check_compose_input(a)&&f.command(a,{nocheck:!0})};e=this.show_popup_dialog(b,this.get_label("nosubjecttitle"),
[{text:this.get_label("sendmessage"),click:function(){s()},"class":"mainaction"},{text:this.get_label("cancel"),click:function(){n.focus();e.dialog("close")}}],{dialogClass:"warning"});t.select().keydown(function(a){13==a.which&&s()});this.env.nosubject_warned=!0;return!1}if(!this.mailvelope_editor&&!this.editor.get_content()&&!confirm(this.get_label("nobodywarning")))return this.editor.focus(),!1;this.editor.save();return!0};this.toggle_editor=function(a,b,d){b=this.editor.toggle(a.html,a.noconvert||
...
"object"===typeof a[e]?a[e].display||a[e].name:a[e],l="object"===typeof a[e]?a[e].type:"",g=e+this.env.contacts.length,$("<li>").attr("id","rcmkSearchItem"+g).attr("role","option").html('<i class="icon"></i>'+this.quote_html(k.replace(RegExp("("+RegExp.escape(m)+")","ig"),"##$1%%")).replace(/##([^%]+)%%/g,"<b>$1</b>")).addClass(l||"").appendTo(b).mouseover(function(){f.ksearch_select(this)}).mouseup(function(){f.ksearch_click(this)}).get(0)._rcm_id=g,n-=1;b.childNodes.length&&($(this.ksearch_input).attr("aria-haspopup",
...
this.ksearch_msg=this.ksearch_info=this.ksearch_data=null};this.contactlist_keypress=function(a){a.key_pressed==a.DELETE_KEY&&this.command("delete")};this.contactlist_select=function(a){this.preview_timer&&clearTimeout(this.preview_timer);var b,d,e,g,h=!1,k=a.selection.length,l=this.env.source?this.env.address_sources[this.env.source]:null;this.env.contentframe&&!a.multi_selecting&&(d=a.get_single_selection())?this.preview_timer=setTimeout(function(){f.load_contact(d,"show")},this.preview_delay_click):
this.env.contentframe&&this.show_contentframe(!1);if(k){a.draggable=!1;this.env.selection_sources=[];l&&this.env.selection_sources.push(this.env.source);for(b in a.selection)g=a.data[a.selection[b]],l?h=h||!l.readonly&&!g.readonly:(e=String(a.selection[b]).replace(/^[^-]+-/,""))&&this.env.address_sources[e]&&(h=h||!this.env.address_sources[e].readonly&&!g.readonly,this.env.selection_sources.push(e)),"group"!=g._type&&(a.draggable=!0);this.env.selection_sources=$.unique(this.env.selection_sources)}this.enable_command("group-remove-selected",
...
s,t=[],q=[];if(h=this.env[a+"_template"])for(k=0;k<h.length;k++)t.push(h[k][1]),q.push(h[k][2]);else for(p in b.childs)t.push(p);for(k=0;k<t.length;k++)p=t[k],h=b.childs[p],h=$("<input>").addClass("ff_"+p).attr({type:"text",name:"_"+p+n,size:h.size}).appendTo(l),l.append(q[k]||" "),this.init_edit_field(p,h),s||(s=h);h=s}else if("select"==b.type){h=$("<select>").addClass("ff_"+a).attr({name:"_"+a+n,id:k}).appendTo(l);var u=h.attr("options");u[u.length]=new Option("---","");b.options&&$.each(b.options,
function(a,b){u[u.length]=new Option(b,a)})}h&&($('<a href="#del"></a>').addClass("contactfieldbutton deletebutton").attr({title:this.get_label("delete"),rel:a}).html(this.env.delbutton).click(function(){f.delete_edit_field(this);return!1}).appendTo(l),g.append(m).append(l).appendTo(e.show()),h.first().focus(),b.count||(b.count=0),++b.count==b.limit&&b.limit&&$(d).children('option[value="'+a+'"]').prop("disabled",!0))}}};this.delete_edit_field=function(a){var b=$(a).attr("rel"),d=this.env.coltypes[b],
...
this.subscription_list.is_search()&&(this.subscription_select(),this.subscription_list.reset_search());this.subscription_list.draggable("destroy").droppable("destroy");var m,n,p,s,t,q="",u=[],w=[],v=[],x=$(this.gui_objects.subscriptionlist);m=k?k:$($("li",x).get(1)).clone(!0);if(!m.length)return this.goto_url("folders"),!1;m.attr({id:"rcmli"+this.html_identifier_encode(a),"class":h});k&&k.length||($("ul,div.treetoggle",m).remove(),m.removeData("filtered"));$("a:first",m).text(d);$('input[name="_subscribed[]"]:first',
m).val(a).prop({checked:g?!0:!1,disabled:e?!0:!1});this.env.subscriptionrows[a]=[b,d,!1];$.each(this.env.subscriptionrows,function(a,b){b[3]=a;u.push(b)});try{t=new Intl.Collator(this.env.locale.replace("_","-"))}catch(y){}u.sort(function(a,b){var d,e,g,h=a[0].split(f.env.delimiter),k=b[0].split(f.env.delimiter),l=h.length;for(d=0;d<l;d++){e=h[d];g=k[d];if(e!==g)return void 0===g?1:t?t.compare(e,g):e<g?-1:1;if(d==l-1)return-1}});for(n in u)d=u[n][3],u[n][2]?(b=d+this.env.delimiter,b!=this.env.prefix_ns&&
(v.push(d),p=b)):p&&d.startsWith(p)?v.push(d):(w.push(d),p=null);for(n=0;n<v.length;n++)a.startsWith(v[n]+this.env.delimiter)&&(s=v[n]);for(n=0;!s&&n<w.length;n++)n&&w[n]==a&&(s=w[n-1]);if(s&&(n=this.subscription_list.get_item(s,!0))){if(p=a.lastIndexOf(this.env.delimiter))q=a.substring(0,p),q=this.subscription_list.get_item(q,!0),$("div.treetoggle",q).length||$("<div>&nbsp;</div>").addClass("treetoggle collapsed").appendTo(q),$("ul",q).length||$("<ul>").css("display","none").appendTo(q);if(q&&n==
q)$("ul:first",q).append(m);else{for(;(d=$(n).parent().parent().get(0))&&(!q||d!=q)&&$(d).is("li.mailbox");)n=d;$(n).after(m)}}else x.append(m);$.extend(this.env.subscriptionrows,l||{});this.subscription_list.reset(!0);this.subscription_select();q&&this.subscription_list.expand(this.folder_id2name(q.id));m=m.show().get(0);m.scrollIntoView&&m.scrollIntoView();return m};this.replace_folder_row=function(a,b,d,e,g,h){if(!this.gui_objects.subscriptionlist)return this.is_framed()?window.parent.rcmail.replace_folder_row(a,
...
this.env.delimiter)&&(e+=this.env.unread_counts[l]);f&&d.length?d.html(this.env.unreadwrap.replace(/%[sd]/,f)):d.length&&d.remove();d=RegExp(RegExp.escape(this.env.delimiter)+"[^"+RegExp.escape(this.env.delimiter)+"]+$");a.match(d)&&this.set_unread_count_display(a.replace(d,""),!1);0<f+e?$(g).addClass("unread"):$(g).removeClass("unread")}d=/^\([0-9]+\)\s+/i;b&&document.title&&(g="",g=String(document.title),g=f&&g.match(d)?g.replace(d,"("+f+") "):f?"("+f+") "+g:g.replace(d,""),this.set_pagetitle(g))};
...
d=$('<div id="folder-selector" class="popupmenu"></div>');k.href="#";k.className="icon";$.each(this.env.mailboxes_list,function(){var a=0,b=0,d=f.env.mailboxes[this],h=d.id,s=$(k.cloneNode(!1)),t=$("<li>");d.virtual?s.addClass("virtual").attr("aria-disabled","true").attr("tabindex","-1"):s.addClass("active").data("id",d.id);for(d["class"]&&s.addClass(d["class"]);0<=(b=h.indexOf(g,b));)a++,b++;s.css("padding-left",a?16*a+"px":0);s.append($("<span>").text(d.name));t.append(s);e.push(t)});h.append(e).appendTo(d);
d.css({left:"-1000px",top:"-1000px"}).appendTo($("body")).show();10<e.length&&d.css("max-height",10*$("li",d)[0].offsetHeight+9);d.on("click","a.active",function(a){d.data("callback")($(this).data("id"));return!1});this.folder_selector_element=d}d.data("callback",b);this.show_menu("folder-selector",!0,a)};this.show_menu=function(a,b,d){var e="object"==typeof a?a.menu:a,f=$("#"+e),h=d&&d.target?$(d.target):$(f.attr("rel")||"#"+e+"link"),k=rcube_event.is_keyboard(d),l=f.attr("data-align")||"",m=!1;
"A"!=h.get(0).tagName&&h.closest("a").length&&(h=h.closest("a"));"string"==typeof a&&(a={menu:e});f.length||(f=this.triggerEvent("menu-get",{name:e,props:a,originalEvent:d}));if(!f||!f.length)return this.triggerEvent(!1===b?"menu-close":"menu-open",{name:e,props:a,originalEvent:d});f.appendTo(document.body);"undefined"==typeof b&&(b=f.is(":visible")?!1:!0);if(b&&h.length){var n=$(window),p=h.offset(),s=0<=l.indexOf("bottom"),m="menuitem"==h.attr("role")||0<h.closest("[role=menuitem]").length;h.offsetWidth=
h.outerWidth();h.offsetHeight=h.outerHeight();!s&&p.top+h.offsetHeight+f.height()>n.height()&&(s=!0);0<=l.indexOf("right")?p.left=p.left+h.outerWidth()-f.width():m&&(p.left=p.left+h.offsetWidth-5,p.top-=h.offsetHeight);p.left+f.width()>n.width()&&(p.left=n.width()-f.width()-12);p.top=Math.max(0,p.top+(s?-f.height():h.offsetHeight));f.css({left:p.left+"px",top:p.top+"px"})}if(b){for(l=this.menu_stack.length-1;m&&0<=l;l--)$(h).parents("#"+this.menu_stack[l]).length||"menuitem"==$(d.target).parent().attr("role")||
...
this.file_drag_hover=function(a,b){a.preventDefault();a.stopPropagation();$(this.gui_objects.filedrop)[b?"addClass":"removeClass"]("hover")};this.file_dropped=function(a){this.file_drag_hover(a,!1);var b,d=0,e=0,g=a.target.files||a.dataTransfer.files,h=window.FormData?new FormData:null,k=(this.env.filedrop.fieldname||"_file")+(this.env.filedrop.single?"":"[]"),l="------multipartformboundary"+(new Date).getTime(),m="--"+l+"\r\n",n={_id:this.env.compose_id||this.env.cid||"",_remote:1,_from:this.env.action};
if(g&&g.length)for(var p=function(){if(f.env.max_filesize&&f.env.filesizeerror&&d>f.env.max_filesize)f.display_message(f.env.filesizeerror,"error");else if(f.env.max_filecount&&f.env.filecounterror&&e>f.env.max_filecount)f.display_message(f.env.filecounterror,"error");else{var a=1<g.length,b=(new Date).getTime(),a=$("<span>").text(a?f.get_label("uploadingmany"):g[0].name).html();f.add2attachment_list(b,{name:"",html:a,classname:"uploading",complete:!1})||(f.file_upload_id=f.set_busy(!0,"uploading"));
m+="--"+l+"--\r\n";n._uploadid=b;$.ajax({type:"POST",dataType:"json",url:f.url(f.env.filedrop.action||"upload",n),contentType:h?!1:"multipart/form-data; boundary="+l,processData:!1,timeout:0,data:h||m,headers:{"X-Roundcube-Request":f.env.request_token},xhr:function(){var a=jQuery.ajaxSettings.xhr();!h&&a.sendAsBinary&&(a.send=a.sendAsBinary);return a},success:function(a){f.http_response(a)},error:function(a,b,d){f.http_error(a,b,d,null,"attachment")}})}},s=this.env.filedrop.single?0:g.length-1,t=
b=0,q;b<=s&&(q=g[t]);t++){if(q.name||(q.name=q.fileName),q.size||(q.size=q.fileSize),q.type||(q.type="application/octet-stream"),!h&&/[^\x20-\x7E]/.test(q.name)&&(q.name_bin=unescape(encodeURIComponent(q.name))),!this.env.filedrop.filter||q.type.match(RegExp(this.env.filedrop.filter))){d+=q.size;e++;if(h){if(h.append(k,q),b==s)return p()}else if(window.FileReader){var u=new FileReader;u.onload=function(a,b){return function(d){m+='Content-Disposition: form-data; name="'+k+'"';m+='; filename="'+(q.name_bin||
a.name)+'"\r\n';m+="Content-Length: "+a.size+"\r\n";m+="Content-Type: "+a.type+"\r\n\r\n";m+=u.result+"\r\n";m+="--"+l+"\r\n";if(b==s)return p()}}(q,b);u.readAsBinaryString(q)}else if(q.getAsBinary&&(m+='Content-Disposition: form-data; name="'+k+'"',m+='; filename="'+(q.name_bin||q.name)+'"\r\n',m+="Content-Length: "+q.size+"\r\n",m+="Content-Type: "+q.type+"\r\n\r\n",m+=q.getAsBinary()+"\r\n",m+="--"+l+"\r\n",b==s))return p();b++}}else if(b=a.dataTransfer.getData("roundcube-uri"))t=(new Date).getTime(),
a=$("<span>").text(a.dataTransfer.getData("roundcube-name")||this.get_label("attaching")).html(),n._uri=b,n._uploadid=t,this.add2attachment_list(t,{name:"",html:a,classname:"uploading",complete:!1})||(this.file_upload_id=this.set_busy(!0,"attaching")),this.http_post(this.env.filedrop.action||"upload",n)};this.start_keepalive=function(){if(this.env.session_lifetime&&!this.env.framed&&!this.env.extwin&&"login"!=this.task&&"print"!=this.env.action){this._keepalive&&clearInterval(this._keepalive);var a=
500*Math.min(1800,this.env.session_lifetime);this._keepalive=setInterval(function(){f.keep_alive()},3E4>a?3E4:a)}};this.start_refresh=function(){!this.env.refresh_interval||this.env.framed||this.env.extwin||"login"==this.task||"print"==this.env.action||(this._refresh&&clearInterval(this._refresh),this._refresh=setInterval(function(){f.refresh()},1E3*this.env.refresh_interval))};this.keep_alive=function(){this.busy||this.http_request("keep-alive")};this.refresh=function(){if(this.busy)setTimeout(function(){f.refresh();
...
this.image_support_check=function(a){window.setTimeout(function(){var b=new Image;b.onload=function(){f.env.browser_capabilities[a]=1};b.onerror=function(){f.env.browser_capabilities[a]=0};b.src=f.assets_path("program/resources/blank."+a)},10)};this.pdf_support_check=function(){var a,b=navigator.mimeTypes?navigator.mimeTypes["application/pdf"]:{},d=navigator.plugins,e=d.length,g=/Adobe Reader|PDF|Acrobat/i;if(b&&b.enabledPlugin)return 1;if("ActiveXObject"in window){try{if(b=new ActiveXObject("AcroPDF.PDF"))return 1}catch(h){}try{if(b=
new ActiveXObject("PDF.PdfCtrl"))return 1}catch(k){}}for(a=0;a<e;a++)if(b=d[a],"String"===typeof b){if(g.test(b))return 1}else if(b.name&&g.test(b.name))return 1;window.setTimeout(function(){$("<object>").attr({data:f.assets_path("program/resources/dummy.pdf"),type:"application/pdf",style:'position: "absolute"; top: -1000px; height: 1px; width: 1px'}).on("load error",function(a){f.env.browser_capabilities.pdf="load"==a.type?1:0;$(this).remove()}).appendTo($("body"))},10);return 0};this.flash_support_check=
function(){var a=navigator.mimeTypes?navigator.mimeTypes["application/x-shockwave-flash"]:{};if(a&&a.enabledPlugin)return 1;if("ActiveXObject"in window)try{if(new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))return 1}catch(b){}return 0};this.assets_path=function(a){this.env.assets_path&&!a.startsWith(this.env.assets_path)&&(a=this.env.assets_path+a);return a};this.set_cookie=function(a,b,d){setCookie(a,b,d,this.env.cookie_path,this.env.cookie_domain,this.env.cookie_secure)};this.get_local_storage_prefix=
function(){this.local_storage_prefix||(this.local_storage_prefix="roundcube."+(this.env.user_id||"anonymous")+".");return this.local_storage_prefix};this.local_storage_get_item=function(a,b,d){var e,f;try{e=localStorage.getItem(this.get_local_storage_prefix()+a),f=JSON.parse(e)}catch(h){}return f||b||null};this.local_storage_set_item=function(a,b,d){try{return localStorage.setItem(this.get_local_storage_prefix()+a,JSON.stringify(b)),!0}catch(e){return!1}};this.local_storage_remove_item=function(a){try{return localStorage.removeItem(this.get_local_storage_prefix()+
a),!0}catch(b){return!1}};this.print_dialog=function(){bw.safari?setTimeout("window.print()",10):window.print()}}rcube_webmail.long_subject_title=function(f,v){if(!f.title){var a=$(f);a.width()+15*(v||0)>a.parent().width()&&(f.title=rcube_webmail.subject_text(f))}};
...
rcube_webmail.set_iframe_events=function(f){$("iframe").each(function(){var v=$(this);$.each(f,function(a,b){v.on("load",function(d){try{$(this).contents().on(a,b)}catch(f){}});try{v.contents().on(a,b)}catch(d){}})})};rcube_webmail.prototype.get_cookie=getCookie;rcube_webmail.prototype.addEventListener=rcube_event_engine.prototype.addEventListener;rcube_webmail.prototype.removeEventListener=rcube_event_engine.prototype.removeEventListener;rcube_webmail.prototype.triggerEvent=rcube_event_engine.prototype.triggerEvent;
