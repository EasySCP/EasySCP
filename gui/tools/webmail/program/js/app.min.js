...
" "):d.seen?0<d.unread_children&&(l+=" unreadchildren"):(l+=" unread",m+=this.get_label("unread")+" "));d.answered&&(l+=" replied",m+=this.get_label("replied")+" ");d.forwarded&&(l+=" forwarded",m+=this.get_label("forwarded")+" ");f.selected&&!q.in_selection(a)&&q.selection.push(a);this.env.threading&&(f.depth?(n+='<span id="rcmtab'+t+'" class="branch" style="width:'+15*f.depth+'px;">&nbsp;&nbsp;</span>',s[f.parent_uid]&&!1===s[f.parent_uid].expanded||!(0!=this.env.autoexpand_threads&&2!=this.env.autoexpand_threads||
s[f.parent_uid]&&s[f.parent_uid].expanded)?(w.style.display="none",f.expanded=!1):f.expanded=!0,v+=" thread expanded"):f.has_children&&(void 0===f.expanded&&(1==this.env.autoexpand_threads||2==this.env.autoexpand_threads&&f.unread_children)&&(f.expanded=!0),p='<div id="rcmexpando'+w.id+'" class="'+(f.expanded?"expanded":"collapsed")+'">&nbsp;&nbsp;</div>',v+=" thread"+(f.expanded?" expanded":"")),d.unread_children&&d.seen&&!f.expanded&&(v+=" unroot"));n+='<span id="msgicn'+w.id+'" class="'+g+l+'" title="'+
...
if(27!=b.which&&9!=b.which&&13!=b.which&&!d.is(":visible"))return f.show_menu("pagejump-selector",!0,b);if("keydown"==b.type)if(40==b.which)g.length>h&&(this.value=h+=1);else if(38==b.which)1<h&&g.length>h-1&&(this.value=h-=1);else{if(13==b.which)return $(this).change();if(27==b.which||9==b.which)return $(a).val(f.env.current_page)}$("li.selected",e).removeClass("selected");(b=$(g[h-1])).length&&(b.addClass("selected"),$("#pagejump-selector").scrollTop(e.height()/g.length*(h-1)-d.height()/2))}).on("change",
...
a.join(", ")),"warning")}).fail(function(){console.error("Pubkey lookup failed with",arguments);f.hide_message(g);f.display_message("pubkeysearcherror","error");b(!1)})};this.mailvelope_key_import_dialog=function(a){var b=$("<div>").addClass("listing mailvelopekeyimport");$.each(a,function(a,e){var g=$("<div>").addClass("key");e.revoked&&g.addClass("revoked");e.disabled&&g.addClass("disabled");e.expired&&g.addClass("expired");g.append($("<label>").addClass("keyid").text(f.get_label("keyid")));g.append($("<a>").text(e.keyid.substr(-8).toUpperCase()).attr("href",
...
function(a){a.query&&f.subscription_select()}).draggable({cancel:"li.mailbox.root,input,div.treetoggle"}).droppable({accept:function(a){if(!$(a).is(".mailbox"))return!1;a=f.folder_id2name($(a).attr("id"));var d=f.folder_id2name(this.id),e=f.env.subscriptionrows[a];return e&&!e[2]&&d!=a.replace(f.last_sub_rx,"")&&!d.startsWith(a+f.env.delimiter)},drop:function(a,d){var e=f.folder_id2name(d.draggable.attr("id")),g=f.folder_id2name(this.id);f.subscription_move_folder(e,g)}})};this.folder_id2name=function(a){return a?
