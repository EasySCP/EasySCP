...
break;default:f=a.replace(/-/g,"_"),this[f]&&"function"===typeof this[f]&&(g=this[f](b,d))}m||!1!==this.triggerEvent("after"+a,b)||(g=!1);this.triggerEvent("actionafter",{props:b,action:a,aborted:m});return!1===g?!1:d?!1:!0};this.enable_command=function(){var a,b,d=Array.prototype.slice.call(arguments),e=d.pop(),g;for(b=0;b<d.length;b++)if(g=d[b],"string"===typeof g)this.commands[g]=e,this.set_button(g,e?"act":"pas"),this.triggerEvent("enable-command",{command:g,status:e});else for(a in g)d.push(g[a])};
...
d.depth:0,unread_children:d.unread_children?d.unread_children:0,parent_uid:d.parent_uid?d.parent_uid:0,selected:this.select_all_mode||this.message_list.in_selection(a),ml:d.ml?1:0,ctype:d.ctype,flags:d.extra_flags});var g,f,h,m="",k="",l=this.message_list,p=l.rows;h=this.env.messages[a];var q="message"+(d.seen?"":" unread")+(d.deleted?" deleted":"")+(d.flagged?" flagged":"")+(h.selected?" selected":""),u={cols:[],style:{},id:"rcmrow"+a};g="msgicon";null===this.env.status_col&&(g+=" status",d.deleted?
g+=" deleted":d.seen?0<d.unread_children&&(g+=" unreadchildren"):g+=" unread");d.answered&&(g+=" replied");d.forwarded&&(g+=" forwarded");h.selected&&!l.in_selection(a)&&l.selection.push(a);this.env.threading&&(h.depth?(m+='<span id="rcmtab'+a+'" class="branch" style="width:'+15*h.depth+'px;">&nbsp;&nbsp;</span>',p[h.parent_uid]&&!1===p[h.parent_uid].expanded||!(0!=this.env.autoexpand_threads&&2!=this.env.autoexpand_threads||p[h.parent_uid]&&p[h.parent_uid].expanded)?(u.style.display="none",h.expanded=
!1):h.expanded=!0,q+=" thread expanded"):h.has_children&&(void 0===h.expanded&&(1==this.env.autoexpand_threads||2==this.env.autoexpand_threads&&h.unread_children)&&(h.expanded=!0),k='<div id="rcmexpando'+a+'" class="'+(h.expanded?"expanded":"collapsed")+'">&nbsp;&nbsp;</div>',q+=" thread"+(h.expanded?" expanded":"")),d.unread_children&&d.seen&&!h.expanded&&(q+=" unroot"));m+='<span id="msgicn'+a+'" class="'+g+'">&nbsp;</span>';u.className=q;b.subject&&(g=d.mbox==this.env.drafts_mailbox?"_draft_uid":
...
this.permanently_remove_messages();return!0};this.permanently_remove_messages=function(){var a=this.selection_post_data();a._uid&&(this.show_contentframe(!1),this._with_selected_messages("delete",a))};this._with_selected_messages=function(a,b,d){var e=0;if(this.message_list){var g,f,h,m=[],k=this.message_list.get_selection();g=0;for(len=k.length;g<len;g++)f=k[g],this.env.threading&&(e+=this.update_thread(f),h=this.message_list.find_root(f),h!=f&&0>$.inArray(h,m)&&m.push(h)),this.message_list.remove_row(f,
this.env.display_next&&g==k.length-1);this.env.display_next||this.message_list.clear_selection();g=0;for(len=m.length;g<len;g++)this.add_tree_icons(m[g])}0>e?b._count=-1*e:0<e&&this.delete_excessive_thread_rows();d||(d=this.display_message(this.get_label("move"==a?"movingmessage":"deletingmessage"),"loading"));this.http_post(a,b,d)};this.selection_post_data=function(a){"object"!=typeof a&&(a={});a._mbox=this.env.mailbox;if(!a._uid){var b=this.env.uid?[this.env.uid]:this.message_list.get_selection();
a._uid=this.uids_to_list(b)}this.env.action&&(a._from=this.env.action);this.env.search_request&&(a._search=this.env.search_request);this.env.display_next&&this.env.next_uid&&(a._next_uid=this.env.next_uid);return a};this.mark_message=function(a,b){var d=[],e=[],g,f,h,m=this.message_list;b?d[0]=b:this.env.uid?d[0]=this.env.uid:m&&(d=m.get_selection());if(m)for(m.focus(),f=0,g=d.length;f<g;f++)h=d[f],("read"==a&&m.rows[h].unread||"unread"==a&&!m.rows[h].unread||"delete"==a&&!m.rows[h].deleted||"undelete"==
a&&m.rows[h].deleted||"flagged"==a&&!m.rows[h].flagged||"unflagged"==a&&m.rows[h].flagged)&&e.push(h);else e=d;if(e.length||this.select_all_mode)switch(a){case "read":case "unread":this.toggle_read_status(a,e);break;case "delete":case "undelete":this.toggle_delete_status(e);break;case "flagged":case "unflagged":this.toggle_flagged_status(a,d)}};this.toggle_read_status=function(a,b){var d,e=b.length,g=this.selection_post_data({_uid:this.uids_to_list(b),_flag:a}),f=this.display_message(this.get_label("markingmessage"),
...
function(a){for(var b=[],d=this.selection_post_data({_uid:this.uids_to_list(a),_flag:"delete"}),e=this.display_message(this.get_label("markingmessage"),"loading"),g=this.message_list?this.message_list.rows:{},f=0,h=0,m=a.length;h<m;h++)uid=a[h],g[uid]&&(g[uid].unread&&(b[b.length]=uid),this.env.skip_deleted?(f+=this.update_thread(uid),this.message_list.remove_row(uid,this.env.display_next&&h==this.message_list.selection.length-1)):this.set_message(uid,"deleted",!0));this.env.skip_deleted&&this.message_list&&
...
e=$("input[name='_subject']"),g=$("[name='_message']").get(0),f="1"==$("input[name='_is_html']").val(),h=["cc","bcc","replyto","followupto"],m,k=this.opener();k&&"compose"==k.env.action&&(setTimeout(function(){1<opener.history.length?opener.history.back():k.redirect(k.get_task_url("mail"))},100),this.env.opened_extwin=!0);0<this.env.autocomplete_threads&&(m={threads:this.env.autocomplete_threads,sources:this.env.autocomplete_sources});this.init_address_input_events(d,m);for(a in h)this.init_address_input_events($("[name='_"+
h[a]+"']"),m);f||(a=this.env.top_posting?0:g.value.length,this.set_caret_pos(g,a),"select-one"==b.prop("type")&&this.change_identity(b[0]),a&&$(g).scrollTop(g.scrollHeight));this.compose_restore_dialog(0,f);""==d.val()?d.focus():""==e.val()?e.focus():g&&g.focus();this.env.compose_focus_elem=document.activeElement;this.compose_field_hash(!0);this.auto_save_start()};this.compose_restore_dialog=function(a,b){var d,e,g,f=this.local_storage_get_item("compose.index",[]),h=function(a){++a<f.length&&k.compose_restore_dialog(a,
...
e=-1;var g=$("[name='_message']"),f=g.val(),h="1"==$("input[name='_is_html']").val(),m=this.env.identity,n=this.env.recipients_separator,l=RegExp.escape(n);$.each(["replyto","bcc"],function(){var a,b=m&&k.env.identities[m]?k.env.identities[m][this]:"",e=d&&k.env.identities[d]?k.env.identities[d][this]:"",g=$('[name="_'+this+'"]'),f=g.val();b&&f&&(a=RegExp("\\s*"+RegExp.escape(b)+"\\s*"),f=f.replace(a,""));a=RegExp(l+"\\s*"+l,"g");f=f.replace(a,n);a=RegExp("^[\\s"+l+"]+");f=f.replace(a,"");e&&-1==
...
h.nextSibling),f.insertBefore(e.createElement("br"),h.nextSibling))):(bw.ie&&f.appendChild(e.createElement("br")),f.appendChild(g))),this.env.signatures[d]&&(g.innerHTML=this.env.signatures[d].html)):(b&&m&&this.env.signatures&&this.env.signatures[m]&&(m=this.env.signatures[m].text,m=m.replace(/\r\n/g,"\n"),e=this.env.top_posting?f.indexOf(m):f.lastIndexOf(m),0<=e&&(f=f.substring(0,e)+f.substring(e+m.length,f.length))),b&&this.env.signatures&&this.env.signatures[d]?(m=this.env.signatures[d].text,
m=m.replace(/\r\n/g,"\n"),this.env.top_posting?0<=e?(f=f.substring(0,e)+m+f.substring(e,f.length),e-=1):f?(pos=this.get_caret_pos(g.get(0)))?(f=f.substring(0,pos)+"\n"+m+"\n\n"+f.substring(pos,f.length),e=pos):(e=0,f="\n\n"+m+"\n\n"+f.replace(/^[\r\n]+/,"")):(e=0,f="\n\n"+m):(f=f.replace(/[\r\n]+$/,""),e=!this.env.top_posting&&f.length?f.length+1:0,f+="\n\n"+m)):e=this.env.top_posting?0:f.length,g.val(f),this.set_caret_pos(g.get(0),e));this.env.identity=d;this.triggerEvent("change_identity");return!0};
...
n2=b[d];return n1<n2?-1:1});for(k in p)p[k][2]?(b=p[k][0]+this.env.delimiter,b!=this.env.prefix_ns&&(u.push(p[k][0]),n=b)):n&&p[k][0].startsWith(n)?u.push(p[k][0]):(q.push(p[k][0]),n=null);for(k=0;k<u.length;k++)a.startsWith(u[k]+this.env.delimiter)&&(l=this.get_folder_row_id(u[k]));for(k=0;!l&&k<q.length;k++)k&&q[k]==a&&(l=this.get_folder_row_id(q[k-1]));l?$("#"+l).after(h):h.appendTo(v);this.subscription_list.clear_selection();g||this.init_subscription_list();h=h.get(0);h.scrollIntoView&&h.scrollIntoView();
return h};this.replace_folder_row=function(a,b,d,e,g){if(!this.gui_objects.subscriptionlist)return this.is_framed?parent.rcmail.replace_folder_row(a,b,d,e,g):!1;var f,h,k,n,l=this.get_folder_row_id(a),p=a.length;f=$('input[name="_subscribed[]"]',$("#"+l)).prop("checked");var q=this.get_subfolders(a);if(a==b)$("#"+l).attr("class",g||""),this.subscription_list.focus();else{this._remove_folder_row(l);e=$(this.add_folder_row(b,d,e,f,!0,g));if(d=q.length)n=a.split(this.env.delimiter).length-b.split(this.env.delimiter).length;
for(a=0;a<d;a++)if(l=q[a],f=this.env.subscriptionrows[l][0],g=this.env.subscriptionrows[l][1],h=$("#"+l),k=h.clone(!0),h.remove(),e.after(k),e=k,f=b+f.slice(p),$('input[name="_subscribed[]"]',e).val(f),this.env.subscriptionrows[l][0]=f,0!=n){if(0<n)for(f=n;0<f;f--)g=g.replace(/^&nbsp;&nbsp;&nbsp;&nbsp;/,"");else for(f=n;0>f;f++)g="&nbsp;&nbsp;&nbsp;&nbsp;"+g;e.find("td:first").html(g);this.env.subscriptionrows[l][1]=g}this.init_subscription_list()}};this.remove_folder_row=function(a,b){var d,e,g=
...
($(b).removeClass("hide-headers").addClass("show-headers"),$(this.gui_objects.all_headers_row).hide(),b.onclick=function(){rcmail.command("show-headers","",b)})};this.folder_selector=function(a,b){var d=this.folder_selector_element;if(!d){var e=[],g=this.env.delimiter,f=$('<ul class="toolbarmenu iconized">'),h=document.createElement("li"),m=document.createElement("a"),n=document.createElement("span"),d=$('<div id="folder-selector" class="popupmenu"></div>');m.href="#";m.className="icon";$.each(this.env.mailboxes_list,
function(){var a,b=a=0,f=k.env.mailboxes[this],l=f.id,s=m.cloneNode(!1),t=h.cloneNode(!1);f.virtual?s.className+=" virtual":(s.className+=" active",s.onclick=function(){d.hide().data("callback")(f.id)});f["class"]&&(s.className+=" "+f["class"]);for(;0<=(b=l.indexOf(g,b));)a++,b++;s.style.paddingLeft=a?16*a+"px":0;a=n.cloneNode(!1);$(a).text(f.name);s.appendChild(a);t.appendChild(s);e.push(t)});f.append(e).appendTo(d);d.css({left:"-1000px",top:"-1000px"}).appendTo($("body")).show();10<e.length&&d.css("max-height",
10*$("li",d)[0].offsetHeight+9);var l=function(a){a.target!=d.get(0)&&d.hide()};$(document.body).on("mouseup",l);$("iframe").contents().on("mouseup",l).load(function(a){try{$(this).contents().on("mouseup",l)}catch(b){}});this.folder_selector_element=d}this.element_position(d,a);d.show().data("callback",b)};this.element_position=function(a,b){b=$(b);var d=$(window),e=b.outerWidth(),g=b.outerHeight(),f=b.data("menu-pos"),h=d.height(),k=$(a).height(),n=$(a).width(),l=b.offset(),p=l.top,l=l.left+e;"bottom"==
f?(p+=g,l-=e):l-=5;p+k>h&&(p-=k-g,0>p&&(p=Math.max(0,(h-k)/2)));l+n>d.width()&&(l-=n+e);a.css({left:l+"px",top:p+"px"})};this.html2plain=function(a,b){if(!a||!a.replace(/<[^>]+>|&nbsp;|\s/g,"").length)return setTimeout(function(){$("#"+b).val("")},50),!0;if(!confirm(this.get_label("editorwarning")))return!1;var d=this,e=this.set_busy(!0,"converting");this.log("HTTP POST: ?_task=utils&_action=html2text");$.ajax({type:"POST",url:"?_task=utils&_action=html2text",data:a,contentType:"application/octet-stream",
...
null;case "list":"mail"==this.task?(this.enable_command("show","select-all","select-none",0<this.env.messagecount),this.enable_command("expunge",this.env.exists),this.enable_command("purge",this.purge_mailbox_test()),this.enable_command("expand-all","expand-unread","collapse-all",this.env.threading&&this.env.messagecount),"list"!=a.action&&"search"!=a.action||!this.message_list||(this.message_list.triggerEvent("select"),this.triggerEvent("listupdate",{folder:this.env.mailbox,rowcount:this.message_list.rowcount}))):
...
{ts:g},d);$(a).attr({target:f,action:this.url(b,{_id:this.env.compose_id||"",_uploadid:g}),method:"POST"}).attr(a.encoding?"encoding":"enctype","multipart/form-data").submit();return f};this.document_drag_hover=function(a,b){$(this.gui_objects.filedrop)[b?"addClass":"removeClass"]("active")};this.file_drag_hover=function(a,b){a.preventDefault();a.stopPropagation();$(k.gui_objects.filedrop)[b?"addClass":"removeClass"]("hover")};this.file_dropped=function(a){this.file_drag_hover(a,!1);var b=a.target.files||
a.dataTransfer.files,d=window.FormData?new FormData:null,e=(this.env.filedrop.fieldname||"_file")+(this.env.filedrop.single?"":"[]"),g="------multipartformboundary"+(new Date).getTime(),f="--"+g+"\r\n";if(b&&b.length)for(var h=function(){var a=1<b.length,e=(new Date).getTime(),a=$("<span>").text(a?k.get_label("uploadingmany"):b[0].name).html();k.add2attachment_list(e,{name:"",html:a,classname:"uploading",complete:!1})||(k.file_upload_id=k.set_busy(!0,"uploading"));f+="--"+g+"--\r\n";$.ajax({type:"POST",
dataType:"json",url:k.url(k.env.filedrop.action||"upload",{_id:k.env.compose_id||k.env.cid||"",_uploadid:e,_remote:1}),contentType:d?!1:"multipart/form-data; boundary="+g,processData:!1,timeout:0,data:d||f,headers:{"X-Roundcube-Request":k.env.request_token},xhr:function(){var a=jQuery.ajaxSettings.xhr();!d&&a.sendAsBinary&&(a.send=a.sendAsBinary);return a},success:function(a){k.http_response(a)},error:function(a,b,d){k.http_error(a,b,d,null,"attachment")}})},m=this.env.filedrop.single?0:b.length-
1,n=a=0,l;a<=m&&(l=b[n]);n++)if(l.name||(l.name=l.fileName),l.size||(l.size=l.fileSize),l.type||(l.type="application/octet-stream"),!d&&/[^\x20-\x7E]/.test(l.name)&&(l.name_bin=unescape(encodeURIComponent(l.name))),!this.env.filedrop.filter||l.type.match(RegExp(this.env.filedrop.filter))){if(d){if(d.append(e,l),a==m)return h()}else if(window.FileReader){var p=new FileReader;p.onload=function(a,b){return function(d){f+='Content-Disposition: form-data; name="'+e+'"';f+='; filename="'+(l.name_bin||a.name)+
'"\r\n';f+="Content-Length: "+a.size+"\r\n";f+="Content-Type: "+a.type+"\r\n\r\n";f+=p.result+"\r\n";f+="--"+g+"\r\n";if(b==m)return h()}}(l,a);p.readAsBinaryString(l)}else if(l.getAsBinary&&(f+='Content-Disposition: form-data; name="'+e+'"',f+='; filename="'+(l.name_bin||l.name)+'"\r\n',f+="Content-Length: "+l.size+"\r\n",f+="Content-Type: "+l.type+"\r\n\r\n",f+=l.getAsBinary()+"\r\n",f+="--"+g+"\r\n",a==m))return h();a++}};this.start_keepalive=function(){!this.env.session_lifetime||this.env.framed||
...
this.browser_capabilities_check=function(){this.env.browser_capabilities||(this.env.browser_capabilities={});$.each(["pdf","flash","tif"],function(){void 0===k.env.browser_capabilities[this]&&(k.env.browser_capabilities[this]=k[this+"_support_check"]())})};this.browser_capabilities=function(){if(!this.env.browser_capabilities)return"";var a,b=[];for(a in this.env.browser_capabilities)b.push(a+"="+this.env.browser_capabilities[a]);return b.join()};this.tif_support_check=function(){window.setTimeout(function(){var a=
new Image;a.onload=function(){k.env.browser_capabilities.tif=1};a.onerror=function(){k.env.browser_capabilities.tif=0};a.src="program/resources/blank.tif"},10);return 0};this.pdf_support_check=function(){var a,b=navigator.mimeTypes?navigator.mimeTypes["application/pdf"]:{},d=navigator.plugins,e=d.length,g=/Adobe Reader|PDF|Acrobat/i;if(b&&b.enabledPlugin)return 1;if("ActiveXObject"in window){try{if(axObj=new ActiveXObject("AcroPDF.PDF"))return 1}catch(f){}try{if(axObj=new ActiveXObject("PDF.PdfCtrl"))return 1}catch(h){}}for(a=
0;a<e;a++)if(b=d[a],"String"===typeof b){if(g.test(b))return 1}else if(b.name&&g.test(b.name))return 1;window.setTimeout(function(){$("<object>").css({position:"absolute",left:"-10000px"}).attr({data:"program/resources/dummy.pdf",width:1,height:1,type:"application/pdf"}).load(function(){k.env.browser_capabilities.pdf=1}).error(function(){k.env.browser_capabilities.pdf=0}).appendTo($("body"))},10);return 0};this.flash_support_check=function(){var a=navigator.mimeTypes?navigator.mimeTypes["application/x-shockwave-flash"]:
{};if(a&&a.enabledPlugin)return 1;if("ActiveXObject"in window)try{if(axObj=new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))return 1}catch(b){}return 0};this.set_cookie=function(a,b,d){setCookie(a,b,d,this.env.cookie_path,this.env.cookie_domain,this.env.cookie_secure)};this.get_local_storage_prefix=function(){this.local_storage_prefix||(this.local_storage_prefix="roundcube."+(this.env.user_id||"anonymous")+".");return this.local_storage_prefix};this.local_storage_get_item=function(a,b,d){var e;
try{e=localStorage.getItem(this.get_local_storage_prefix()+a)}catch(g){}return null!==e?JSON.parse(e):b||null};this.local_storage_set_item=function(a,b,d){try{return localStorage.setItem(this.get_local_storage_prefix()+a,JSON.stringify(b)),!0}catch(e){return!1}};this.local_storage_remove_item=function(a){try{return localStorage.removeItem(this.get_local_storage_prefix()+a),!0}catch(b){return!1}}}
rcube_webmail.long_subject_title=function(k,t){if(!k.title){var a=$(k);a.width()+15*t>a.parent().width()&&(k.title=a.text())}};rcube_webmail.long_subject_title_ex=function(k){if(!k.title){var t=$(k),a=$.trim(t.text()),b=$("<span>").text(a).css({position:"absolute","float":"left",visibility:"hidden","font-size":t.css("font-size"),"font-weight":t.css("font-weight")}).appendTo($("body")),d=b.width();b.remove();d+15*$("span.branch",t).width()>t.width()&&(k.title=a)}};
