...
if("mail"==rcmail.env.task){rcmail.addEventListener("menu-open",y).addEventListener("menu-close",y).addEventListener("menu-save",O).addEventListener("enable-command",L).addEventListener("responseafterlist",function(a){x(rcmail.env.threading?"thread":"list",!0)}).addEventListener("responseaftersearch",function(a){x(rcmail.env.threading?"thread":"list",!0)});var a=$("#dragmessagemenu");a.length&&(rcmail.gui_object("dragmenu","dragmessagemenu"),w.dragmenu=a);if("show"==rcmail.env.action||"preview"==
rcmail.env.action)rcmail.addEventListener("aftershow-headers",function(){u()}).addEventListener("afterhide-headers",function(){u()}),$("#previewheaderstoggle").click(function(a){H();this.blur&&!rcube_event.is_keyboard(a)&&this.blur();return!1}),$("#attachment-list > li").each(function(){$(this).append($('<a class="drop" tabindex="0" aria-haspopup="true">Show options</a>').on("click keypress",function(a){if("keypress"!=a.type||13==rcube_event.get_keycode(a))return N(this,a),!1}))}),"1"==f("previewheaders")&&
H();else if("compose"==rcmail.env.action){rcmail.addEventListener("aftersend-attachment",z).addEventListener("aftertoggle-editor",function(a){window.setTimeout(function(){s()},200);a&&a.mode&&$("select[name='editorSelector']").val(a.mode)}).addEventListener("compose-encrypted",function(a){$("select[name='editorSelector']").prop("disabled",a.active);$("a.button.attach, a.button.responses")[a.active?"addClass":"removeClass"]("disabled");$("#responseslist a.insertresponse")[a.active?"removeClass":"addClass"]("active")});
for(var d,c,E=["cc","bcc","replyto","followupto"],a=0;a<E.length;a++)d=E[a],c=$("#_"+d),c.length&&(c.on("change",{v:d},function(a){this.value&&C(a.data.v,!0)}),""!=c.val()&&C(d,!0));$("#composeoptionstoggle").click(function(a){var b=$("#composeoptions").toggle().is(":visible");$("#composeoptionstoggle").toggleClass("remove").attr("aria-expanded",b?"true":"false");s();q("composeoptions",b?"1":"0");rcube_event.is_keyboard(a)||this.blur();return!1}).css("cursor","pointer");"0"!==f("composeoptions")&&
$("#composeoptionstoggle").click();$("textarea[name='_to'], textarea[name='_cc'], textarea[name='_bcc']").change(function(a){F(this)}).change();rcmail.addEventListener("autocomplete_insert",function(a){F(a.field)});(a=rcmail.opener())&&"compose"==a.env.action&&$("#composeoptionstoggle",opener.document).hasClass("remove")&&$("#composeoptionstoggle").click();(new rcube_splitter({id:"composesplitterv",p1:"#composeview-left",p2:"#composeview-right",orientation:"v",relative:!0,start:206,min:170,size:12,
...
$("#mailview-bottom").css({top:"auto",height:"0px"}).hide(),n.handle&&n.handle.hide());rcmail.message_list&&(c&&d&&rcmail.message_list.scrollto(d),rcmail.message_list.resize());rcmail.command("save-pref",{name:"preview_pane",value:c?1:0});return!1}),$("#maillistmode").addClass(rcmail.env.threading?"":"selected").click(function(a){x("list");return!1}),$("#mailthreadmode").addClass(rcmail.env.threading?"selected":"").click(function(a){x("thread");return!1}),n=new rcube_splitter({id:"mailviewsplitter",
p1:"#mailview-top",p2:"#mailview-bottom",orientation:"h",relative:!0,start:276,min:150,size:12,offset:4}),a&&n.init(),rcmail.init_pagejumper("#pagejumper"),rcmail.addEventListener("setquota",A).addEventListener("afterimport-messages",z));$("#mailview-left").length&&(new rcube_splitter({id:"mailviewsplitterv",p1:"#mailview-left",p2:"#mailview-right",orientation:"v",relative:!0,start:206,min:150,size:12,callback:b,render:e})).init()}else if("settings"==rcmail.env.task)if(rcmail.addEventListener("init",
...
a=$("body.minwidth");a.length&&a.css("min-width",$(".toolbar").width()+$("#quicksearchbar").width()+$("#searchfilter").width()+30);window.onresize=r;r()};this.init_tabs=J;this.show_about=function(a){var b=$("<iframe>").attr({id:"aboutframe",src:rcmail.url("settings/about"),frameborder:"0"});h=Math.floor(0.75*$(window).height());buttons={};supportln=$("#supportlink");supportln.length&&(v.supporturl=supportln.attr("href"))&&(buttons[supportln.html()]=function(a){0>v.supporturl.indexOf("mailto:")?window.open(v.supporturl):
location.href=v.supporturl});b.dialog({modal:!0,resizable:!1,closeOnEscape:!0,title:a?a.title||a.innerHTML:null,close:function(){b.dialog("destroy").remove()},buttons:buttons,width:640,height:h}).width(640)};this.show_popup=function(a,b,c){!c&&t[a]||B(a,c);c=t[a]||{};c=$(c.link?c.link:"#"+a+"link");var e=c.offset();c.has(".inner")&&(c=c.children(".inner"));return rcmail.command("menu-open",{menu:a,show:b},c.get(0),$.Event("click",{target:c.get(0),pageX:e.left,pageY:e.top,clientX:e.left,clientY:e.top}))};
this.toggle_popup=function(a,b,c){!c&&t[a]||B(a,c);return rcmail.command("menu-open",a,b.target,b)};this.add_popup=B;this.set_searchmod=function(a){var b,c;b=rcmail.env.task;var e=rcmail.env.search_mods,g=rcmail.env.mailbox;"all"==$('input[name="s_scope"]:checked').val()&&(g="*");e||(e={});"mail"==b?(e[g]||(e[g]=rcube_clone_object(e["*"])),c=e[g],b="text"):(c=e,b="*");a.checked?c[a.value]=1:delete c[a.value];a.value==b&&$('input:checkbox[name="s_mods[]"]').map(function(){this!=a&&(this.checked=!0,
a.checked?(this.disabled=!0,delete c[this.value]):(this.disabled=!1,c[this.value]=1))});rcmail.set_searchmods(c)};this.set_searchscope=function(a){rcmail.set_searchscope(a.value)};this.show_uploadform=z;this.show_header_row=C;this.hide_header_row=function(a){var b=$("#_"+a);D[a]=b.val();b.val("");$("#compose-"+a).hide();$("#"+a+"-link").show();s();return!1};this.update_quota=A;this.get_pref=f;this.save_pref=q;this.folder_search_init=G;if(window.$&&document.body){var K=f("minimalmode");(parseInt(K)||
